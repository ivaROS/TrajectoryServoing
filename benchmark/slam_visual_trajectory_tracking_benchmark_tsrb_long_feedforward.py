...
SeqNameList = ['turn','two_circle','loop','zigzag'];
SeqLengList = [15,18,21,36];
# SeqNameList = ['loop'];
# SeqLengList = [21];
...
# 1-5: lambda = 7; 6-10: lambda = 5, new imp
...
reg_enabled_list = [str('true')]
uncert_enabled_list = [str('false')]
feedforward_enabled_list = [str('true')]
odom_topic_list = ['/odom', '/gt_odom', '/visual/odom']
odom_topic_list = ['/gt_odom','/visual/odom']
odom_topic_list = ['/visual/odom']
# odom_topic_list = ['/gt_odom']
param_lambda = 1 # long is 5
param_tau = 10 # 4, 6, 10, 16, 22, 36, 50
                    ...
                    for un, uncert_enabled in enumerate(uncert_enabled_list):
                        for ff, feedforward_enabled in enumerate(feedforward_enabled_list):
                            ...
                            # for iteration in [9]:
                                ...
                                core_name = '/vel_' + str(fv) + '_' + ot + re + ue + ff + '_l' + str(param_lambda) + '_t' + str(param_tau) + '_round' + str(iteration + 1)
                                path_track_logging = slam_save_path + core_name
                                path_map_logging = slam_save_path + core_name + '_Map'
                                # path_logging = Experiment_dir + '/vel_' + str(fv) + '_' + ot + re + ue + ff + '_round' + str(iteration + 1)
                                path_logging = Experiment_dir + core_name
                                ...
                                cmd_ctrl   = str('roslaunch ../launch/controller/' + controller_list[0] + '_controller.launch' \
                                    ...
                                    + ' feat_traj_reg_thresh:=' + str(param_tau))
